...
async function validateRolePassword(role, pin){
  ...
  if(match){
    ...
    await stampRoleLogin(match.row, hashedInput);
    return true;
  }
  return false;
}
async function stampRoleLogin(rowNumber, hashedPin){
  try{
    ...
    await gapi.client.sheets.spreadsheets.values.update({
      ...
      valueInputOption: 'RAW',
      resource: { values: [[email, ROLE_NAME_MAP[_pendingRole] || _pendingRole, hashedPin || '', 'ACTIVE', ts()]] }
    });
  }catch(e){
