...
async function validateRolePassword(role, pin){
  ...
  const match = rows.find(row =>
    row.role === role &&
    row.status === 'ACTIVE' &&
    roleEmailAllowed(row) &&
    String(row.pin) === String(pin)
  );
  if(match){
    getRoleRows._lastPin = match.pin;
...
async function stampRoleLogin(rowNumber){
  try{
    ...
    await gapi.client.sheets.spreadsheets.values.update({
      ...
      resource: { values: [[email, ROLE_NAME_MAP[_pendingRole] || _pendingRole, getRoleRows._lastPin || '', 'ACTIVE', ts()]] }
    });
  }catch(e){
...
async function upsertRolePassword(role, password){
  ...
  const values = [[currentUser?.email || '', roleLabel, password, 'ACTIVE', ts()]];
  if(existing){
    await gapi.client.sheets.spreadsheets.values.update({
      spreadsheetId:SHEET_ID,
