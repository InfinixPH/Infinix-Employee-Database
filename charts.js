...
async function renderDashboard(){
  ...
  // Load log for recent changes display (non-blocking, best-effort)
  if(!logCache){
    try{
    ...
    }catch(e){/* log load failed, use fallback */}
  }
  ...
  // Build recent-changes map from logCache if available
  const recentLogMap={};
  if(logCache){
    const logs=[...logCache].reverse();
    logs.forEach(r=>{
      const empId=String(r[1]||'').trim();
      if(empId&&!recentLogMap[empId]){
        recentLogMap[empId]={action:r[3]||'Updated',from:r[4]||'',to:r[5]||'',time:r[0]||''};
      }
    });
  }
    ...
    <div class="dash-grid">
      <div>
        <div class="charts-row">
          ...
          <!-- DOUGHNUT + STATUS TABLE (matches reference design) -->
                ...
                <table style="width:100%;border-collapse:collapse">
                  <thead>
                    <tr>
                      ...
                      <th style="font-size:8px;font-weight:800;text-transform:uppercase;letter-spacing:1px;color:var(--text3);padding-bottom:6px;border-bottom:1px solid var(--border);text-align:right;width:34px">%</th>
                    </tr>
                  </thead>
                  <tbody>
                  ${STATUSES.map(st=>{
                    ...
                    return`<tr onclick="filterByStatus('${esc(st)}')" style="cursor:pointer">
                      ...
                      <td style="font-size:11.5px;font-weight:600;color:var(--text2);padding:4px 0 4px 6px">${esc(st)}</td>
                      <td style="font-size:12px;font-weight:800;color:${STATUS_COLORS[st]};text-align:right;padding:4px 0">${cnt}</td>
                      <td style="font-size:11px;color:var(--text3);text-align:right;padding:4px 0 4px 6px">${pct}%</td>
                    </tr>`;
                  }).join('')}
                  </tbody>
                </table>
                <!-- Deployment Rate (matches reference) -->
                <div style="margin-top:12px;padding-top:10px;border-top:1px solid var(--border)">
                  <div style="font-size:8.5px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--text3);margin-bottom:4px">Deployment Rate</div>
                  <div style="font-size:22px;font-weight:900;color:#4ecb71;line-height:1">${deployPct}%</div>
          ...
          <div class="chart-card glass-card" style="display:flex;flex-direction:column">
            ...
            <div id="chart-region-info" class="chart-region-info"></div>
          </div>
      ...
      <div class="dash-right">
        <div class="recent-card glass-card">
          ...
          ${recent.map(e=>{
            ...
            // Build change description from log
            const logEntry=recentLogMap[String(e.infinixId||'').trim()];
            let changeDesc='';
            if(logEntry){
              const action=logEntry.action||'Updated';
              const from=logEntry.from;
              const to=logEntry.to;
              if(action==='Added'){changeDesc='New employee added';}
              else if(action==='Deleted'){changeDesc='Record deleted';}
              else if((action==='Status Changed / Moved'||action==='Status Changed')&&from&&to&&from!=='—'&&to&&to!=='—'){changeDesc=`${from} → ${to}`;}
              else if(action==='Updated'&&from&&to&&from!=='—'&&from!==to){changeDesc=`${from} → ${to}`;}
              else{changeDesc=action;}
            } else {
              // Fallback: show status
              changeDesc=e.status&&e.status!=='-'?`Status: ${e.status}`:'Record updated';
            }
            return`<div class="recent-row" onclick="openDetailPanel('${esc(e.infinixId)}')">
              ...
              <div style="flex:1;min-width:0;overflow:hidden">
                ...
                <div style="font-size:10px;color:var(--text3);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(changeDesc)}</div>
              </div>
  ...
  setTimeout(()=>{
    ...
    // BAR CHART — vertical bars sorted by count desc, capped at 70
    const ctxR=document.getElementById('chart-region');
    if(ctxR){
      if(_charts.region)_charts.region.destroy();
      // Sort all regions by count descending (matches design: South Luzon, NCR, Central Luzon, Visayas, Mindanao, North Luzon)
      const allRegions=['SOUTH LUZON','NCR','CENTRAL LUZON','VISAYAS','MINDANAO','NORTH LUZON'];
      // Build entries in correct order: regions that have data sorted desc, then rest
      const rEntries=allRegions
        .map(r=>[r, regions[r]||0])
        .sort((a,b)=>b[1]-a[1]);
      const maxVal=rEntries.length?rEntries[0][1]:0;
      // Cap y-axis at max of 70 or actual max
      const yMax=Math.max(70, Math.ceil(maxVal*1.15));
      const tealSolid=isDark?'rgba(46,196,190,0.88)':'rgba(13,122,118,0.85)';
      const tealDim=isDark?'rgba(46,196,190,0.32)':'rgba(13,122,118,0.30)';
      const barColors=rEntries.map(([,c])=>c===maxVal?tealSolid:tealDim);
      const barBorders=rEntries.map(([,c])=>c===maxVal?(isDark?'rgba(46,196,190,1)':'rgba(13,122,118,1)'):(isDark?'rgba(46,196,190,0.6)':'rgba(13,122,118,0.6)'));
        ...
        options:{
          ...
          scales:{
            x:{
              ...
              ticks:{font:{size:10,weight:'700'},color:tickColor,maxRotation:0,minRotation:0}
            },
            y:{
              beginAtZero:true,
              max:yMax,
              ...
              border:{display:false},
              ticks:{
                font:{size:10,weight:'700'},color:tickColor,precision:0,
      ...
      // Add HIGHEST/LOWEST region labels below chart
      const sorted=[...rEntries].filter(([,c])=>c>0);
      if(sorted.length>=2){
        const highest=sorted[0];
        const lowest=sorted[sorted.length-1];
        const infoEl=document.getElementById('chart-region-info');
        if(infoEl){
          infoEl.innerHTML=`
            <div class="crinfo-item crinfo-high">
              ...
              <div><div class="crinfo-label">HIGHEST REGION</div><div class="crinfo-name">${prettyRegionName(highest[0])}</div><div class="crinfo-sub">${highest[1]} promoters</div></div>
            </div>
            <div class="crinfo-item crinfo-low">
              ...
              <div><div class="crinfo-label">LOWEST REGION</div><div class="crinfo-name">${prettyRegionName(lowest[0])}</div><div class="crinfo-sub">${lowest[1]} promoters</div></div>
            </div>`;
        }
      }
    }
