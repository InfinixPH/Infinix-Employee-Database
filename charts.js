...
function renderTracker(){
  ...
  const storeList=Object.values(byStore).sort((a,b)=>b.total-a.total); // ALL stores, no limit
  ...
  document.getElementById('content').innerHTML=`
    <div class="tracker-date-filters glass-card" style="padding:12px 16px;margin-bottom:16px">
      ...
      ${(trackerDateFrom||trackerDateTo)?`<button class="btn btn-ghost btn-sm" onclick="trackerDateFrom='';trackerDateTo='';renderTracker()">Clear Dates</button>`:''}
    ...
    <div class="section-heading"><span>By Region</span><div class="section-heading-line"></div><span style="font-size:9px;opacity:.55">Click a region card to filter — click again to deselect</span></div>
    <div class="tracker-region-grid" id="tracker-region-grid">
      ${REGIONS.map(r=>{
        const rd=byRegion[r];
        ...
        return`<div class="tracker-region-card glass-card ${isActive?'tracker-region-active':''}" data-region="${esc(r)}" onclick="_trackerToggleRegion(this.dataset.region)" style="cursor:pointer">
          <div class="tracker-region-name"><i class="fi fi-sr-marker" style="font-size:11px;margin-right:5px;color:var(--accent)"></i>${esc(r)}${isActive?` <span style="font-size:9px;opacity:.7;margin-left:4px">[active]</span>`:''}</div>
