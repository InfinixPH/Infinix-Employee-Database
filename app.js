...
// Notification bell — shows pending issues count
function updateNotifBell(){
  try{
    const badge=document.getElementById('notif-count');
    if(!badge||!employees.length)return;
    const activeEmployees=typeof activePromotersOnly==='function'?activePromotersOnly():employees.filter(e=>normalizeStatus(e.status)==='Active');
    const cnt=[
      employees.filter(e=>e._sheet===ACTIVE_SHEET&&isNotYetDeployedColL(e.deploymentStatusColL)).length,
      activeEmployees.filter(e=>!e.contractStatus||e.contractStatus==='NOT YET SENT').length,
      activeEmployees.filter(e=>!requirementsComplete(e)).length,
    ].reduce((a,b)=>a+b,0);
    if(cnt>0){badge.style.display='flex';badge.textContent=cnt>9?'9+':cnt;}
    else{badge.style.display='none';}
  }catch(e){}
}
function onNotifBellClick(){
  if(currentView==='dashboard')showView('active');
}
